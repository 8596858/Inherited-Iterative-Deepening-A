...
class CBSSolver(object):
    ...
    def find_solution(self, disjoint=True):
        ...
        for i in range(self.num_of_agents):  # Find initial path for each agent
            path, temp = a_star(self.my_map, self.starts[i], self.goals[i], self.heuristics[i],
                          i, root['constraints'])
            self.expanded_nodes = temp + self.expanded_nodes
            if path is None:
        ...
        while self.open_list:
            ...
            for constraint in constraints:
                ...
                for p in P['paths']:
                    Q['paths'].append(p)
                a = constraint['agent']
                path, temp = a_star(self.my_map, self.starts[a], self.goals[a], self.heuristics[a], a, Q['constraints'])
                self.expanded_nodes = self.expanded_nodes + temp
                if path is not None:
                    Q['paths'][a] = path
    ...
    def find_solution_IDA(self, disjoint=True):
        ...
        root = {'cost': 0,
                ...
                'collisions': []}
        for i in range(self.num_of_agents):  # Find initial path for each agent
            path, temp = new_IDA(self.my_map, self.starts[i], self.goals[i], self.heuristics[i],
                          i, root['constraints'])
            self.expanded_nodes = self.expanded_nodes + temp
            if path is None:
                raise BaseException('No solutions')
        ...
        while self.open_list:
            ...
            for constraint in constraints:
                ...
                for p in P['paths']:
                    Q['paths'].append(p)
                a = constraint['agent']
                path, temp = new_IDA(self.my_map, self.starts[a], self.goals[a], self.heuristics[a], a, Q['constraints'])
                self.expanded_nodes = self.expanded_nodes + temp
                if path is not None:
                    Q['paths'][a] = path
                    Q['collisions'] = detect_collisions(Q['paths'])
        ...
        while self.open_list:
            ...
            for constraint in constraints:
                ...
                for p in P['paths']:
                    Q['paths'].append(p)
                a = constraint['agent']
                path = ID_a_star(self.my_map, self.starts[a], self.goals[a], self.heuristics[a], a, Q['constraints'])
                if path is not None:
                    Q['paths'][a] = path
                    Q['collisions'] = detect_collisions(Q['paths'])
                    Q['cost'] = get_sum_of_cost(Q['paths'])
    ...
    def find_solution_new_A_star(self, disjoint=True):
        ...
        root = {'cost': 0,
                ...
                'collisions': []}
        for i in range(self.num_of_agents):  # Find initial path for each agent
            path = new_a_star(self.my_map, self.starts[i], self.goals[i], self.heuristics[i],
                          i, root['constraints'])
            if path is None:
                raise BaseException('No solutions')
            root['paths'].append(path)
